...
import { Ban, CheckCircle, ArrowLeft, History } from 'lucide-react'
import Link from 'next/link'
import { EditUserForm } from './edit-form'
import { verifySession } from '@/app/lib/session' // <--- Імпорт

export default async function EditUserPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    const currentUserId = await verifySession(); // <--- Отримуємо ID того, хто дивиться сторінку
    ...
    return (
        <div className="max-w-4xl mx-auto p-4 md:p-6 space-y-6 md:space-y-8">
            <Link href="/dashboard/users" className="text-dark-muted hover:text-white flex items-center gap-2 transition-colors w-fit">
                <ArrowLeft size={20} /> Back to List
            </Link>

            {/* --- ВИПРАВЛЕНА ШАПКА (Mobile Fix) --- */}
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8">
                ...
                <div className="lg:col-span-2 bg-dark-800 border border-dark-700 rounded-2xl p-4 md:p-6">
                    {/* Передаємо currentUserId у форму */}
                ...
                <div className="space-y-6">
                    <div className="bg-dark-800 border border-dark-700 rounded-2xl p-4 md:p-6">
                        <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
                            <History size={20} className="text-primary" /> Recent Access
                        </h3>
